    ...
    return [
      {
        ...
        params: {
          ...
          environmentId: null,
          reason: approval.reason,
          command: stringField(args.command ?? args.cmd) || null,
          cwd: stringField(args.directory ?? args.workdir) || null,
          commandActions: [],
        },
      },
      ...threadStatusChangedNotifications(state, event.threadId),
...
function commandExecutionItem(
  ...
  return {
    type: 'commandExecution',
    id,
    command: stringField(data.command ?? args.command ?? args.cmd),
    cwd: stringField(data.directory ?? args.directory ?? args.workdir) || '.',
