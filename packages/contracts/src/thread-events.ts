...
export function applyRuntimeEventToThread(thread: RuntimeThread, event: StoredThreadEvent): RuntimeThread {
  ...
  // Legacy Collaboration ledger records are decoded by the Feature projection only.
  if (event.type === 'collaboration.task_created'
    || event.type === 'collaboration.task_status_changed') return next;
  ...
  if (event.type === 'approval.requested') {
    ...
    if (message) {
      ...
      upsertToolRun(message, {
        ...
        resultPreview: approval.retryKind !== undefined ? '' : undefined,
        approvalId: approval.id,
        approvalReason: approval.reason,
        approvalReviewer: approval.reviewer ?? 'user',
        approvalRetryKind: approval.retryKind,
        approvalAdditionalPermissions: approval.additionalPermissions,
