...
describe('thread event tool-run projection', () => {
  ...
  it.each(['sandbox_bypass', 'sandbox_readable_root'] as const)(
    'clears the failed first-attempt output when a %s retry awaits approval',
    (retryKind) => {
      const thread: RuntimeThread = {
        id: 'thread_1',
        title: 'Thread',
      ...
      const approvalRequested: RuntimeEvent = {
        ...
        payload: {
          approval: {
            ...
            retryKind,
            additionalPermissions: { file_system: { read: ['C:\\tools\\corepack'] } },
            status: 'pending',
            createdAt: '2026-06-26T00:00:01.000Z',
          },
        },
      ...
      expect(updated.messages[0].toolRuns?.[0]).toMatchObject({
        ...
        resultPreview: '',
        approvalRetryKind: retryKind,
        approvalAdditionalPermissions: { file_system: { read: ['C:\\tools\\corepack'] } },
      });
    },
  );
