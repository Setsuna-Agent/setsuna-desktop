...
export async function createVisionRecognitionCaptureServer(): Promise<{
  baseUrl: string;
  nextRequest: Promise<{ authorization: string; path: string; body: Record<string, unknown> }>;
  close(): Promise<void>;
}> {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      response.write(`data: ${JSON.stringify({ choices: [{ delta: { content: 'The test image was received.' } }] })}\n\n`);
  ...
  await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
  const address = server.address();
  if (!address || typeof address === 'string') throw new Error('Expected TCP address for vision recognition server');
  return {
    baseUrl: `http://127.0.0.1:${address.port}`,
    nextRequest,
    close: () => new Promise((resolve, reject) => server.close((error) => (error ? reject(error) : resolve()))),
