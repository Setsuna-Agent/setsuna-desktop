...
function modelClient(onRequest: (request: ModelRequest) => void = () => undefined, text = 'The image contains one small colored pixel.') {
  return {
...
function runtimeThread(attachments: RuntimeStoredMessageAttachment[]): RuntimeThread {
  return {
    id: 'thread_1',
    title: 'Vision test',
...
function toolHost(
  ...
  fixture?: Awaited<ReturnType<typeof imageFixture>>,
  client = modelClient(),
) {
  ...
  return new OpenAiVisionRecognitionToolHost(
    ...
    plugins,
    {
      async resolveForThread(threadId, attachments) {
        if (!activeFixture || threadId !== activeFixture.thread.id || attachments[0]?.assetId !== activeFixture.attachment.assetId) return [];
        return [{
