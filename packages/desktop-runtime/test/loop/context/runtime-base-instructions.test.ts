...
describe('runtime base instructions', () => {
  it('retains the repository, progress, language, and workspace-reference policies', () => {
    ...
    expect(RUNTIME_BASE_INSTRUCTIONS).toMatch(
      /result-retrieval tool read_tool_result does not require a preamble.*Do not narrate its name.*unless the user explicitly asks or you are debugging the runtime/su,
    );
  });
});
