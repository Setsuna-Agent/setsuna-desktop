...
export class McpRuntimeToolHost implements ToolHost {
  ...
  toolRuntimeProfile(name: string) {
    return { exposure: RESOURCE_TOOL_NAMES.has(name) ? 'direct' as const : 'deferred' as const };
  }

  systemPrompt(): string {
    return [
      'Enabled MCP server tools are exposed as model tools with names prefixed by their server key.',
      'Use list_mcp_resources, list_mcp_resource_templates, and read_mcp_resource for MCP-hosted knowledge that is not exposed as a normal tool.',
