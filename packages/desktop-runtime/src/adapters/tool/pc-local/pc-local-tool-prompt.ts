...
export function pcLocalToolPrompt(
  ...
  if (hasAny(advertised, FILE_MUTATION_TOOL_NAMES)) {
    ...
    lines.push(text('- Reuse current source context for targeted edits. Use the applied result, not the old line numbers, for dependent edits. After compaction, external changes, or a truncated result, read the needed region again. Before structural edits, inspect the enclosing function or section boundaries; a matching anchor alone does not establish the intended location.', '- 定向修改可复用当前源码上下文。后续依赖修改应依据实际应用结果，不能沿用旧行号。上下文压缩、外部修改或结果截断后，重新读取所需区域。结构性修改前检查完整函数或章节边界，不能仅凭锚点匹配就认定位置正确。'));
    if (advertised.has('write_file')) lines.push(text('- Before overwriting an existing file with write_file, use read_file without range arguments and receive the complete, untruncated current contents in this turn. Generate the overwrite only after receiving that result, never alongside the read in the same response. Previews and prior writes do not count as reads; use targeted edits when the file is too large.', '- write_file 覆盖现有文件前，本轮先用 read_file（不指定范围）获取完整、未截断的当前内容。收到读取结果后再生成覆盖内容，不能在同一回复中一起生成读取和覆盖请求。预览和之前的写入不算读取；文件过大时使用定向修改。'));
    lines.push(text('- A successful file edit confirms the write, not code correctness. After a cohesive code change, inspect the affected diff or source and run the narrowest relevant project check before reporting completion. If an edit fails to match, recheck the target and hunk order before retrying; do not guess or remove locating context just to make it apply.', '- 文件编辑成功只表示完成写入，不代表代码正确。完成一组相关代码修改后，检查受影响的差异或源码，并在报告完成前运行项目中最相关的定向检查。修改匹配失败时，先核对目标内容和区块顺序再重试，不要猜测或为了应用成功而去掉定位上下文。'));
  }
  ...
  if (hasAny(advertised, COMMAND_TOOL_NAMES)) {
    ...
    lines.push(
      text('- Discover source files with rg --files and search contents with rg. The desktop runtime puts its managed ripgrep on PATH. Start with the smallest relevant directory and select globs (-g), file-name output (-l), counts (-c), or context (-A/-B/-C) according to the question.', "- 定位源码文件使用 rg --files，搜索内容使用 rg。桌面运行时已将托管的 ripgrep 加入 PATH。从最小相关目录开始，按问题选择 glob（-g）、仅文件名（-l）、计数（-c）或上下文（-A/-B/-C）。"),
      text('- Examples: rg --files . -g "*.ts" finds TypeScript files; rg --files . -g "package.json" finds manifests; rg -n -g "*.ts" "symbolName" . finds matching lines. Replace . with a known relevant directory when possible. Pass globs through -g, not as wildcard paths such as src/*.ts; Windows shells do not expand those paths for rg.', "- 示例：rg --files . -g \"*.ts\" 查找 TypeScript 文件；rg --files . -g \"package.json\" 查找清单；rg -n -g \"*.ts\" \"symbolName\" . 查找匹配行。已知相关目录时，用该目录替换 .。通配符通过 -g 传入，不要把 src/*.ts 当作路径传给 rg；Windows shell 不会替 rg 展开这类路径。"),
      text('- rg --files lists files, not directories. To inspect direct child directories or empty directories, use an available directory-listing tool or the native shell command. Do not use recursive dir /s, Get-ChildItem -Recurse, or custom Node/Python traversal merely to locate source files while rg is available.', "- rg --files 只列文件，不列目录。查看直接子目录或空目录时，使用可用的目录列举工具或当前 shell 的原生命令。rg 可用时，不要仅为定位源码文件就使用递归 dir /s、Get-ChildItem -Recurse 或自行编写 Node/Python 遍历脚本。"),
      text('- Fall back to platform search commands only after rg itself fails to launch or is confirmed missing. No matches, a bad glob/path, or a missing command elsewhere in a pipeline does not mean rg is unavailable; correct the query or pipeline first.', "- 只有 rg 本身无法启动或确认缺失时，才回退到平台搜索命令。没有匹配、glob/路径错误或管道中其他命令缺失，都不代表 rg 不可用；先修正查询或管道。"),
