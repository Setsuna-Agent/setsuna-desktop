...
export function pcLocalToolPrompt(
  ...
  if (advertised.has('search_text')) {
    lines.push(
      text('- Prefer search_text for workspace content search instead of shell grep/find. It uses the runtime-managed ripgrep path and shared ignore policy.', "- 工作区内容搜索优先用 search_text，它使用运行时管理的 ripgrep 路径和统一的忽略规则，而不是 shell grep/find。"),
