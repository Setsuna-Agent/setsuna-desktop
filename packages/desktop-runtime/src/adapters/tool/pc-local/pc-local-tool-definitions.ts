  ...
  return [
    ...
    localTool(
      ...
      [
        ...
        text('Use nearby unchanged lines to identify the edit. An optional @@ followed by a source line, such as a function declaration, searches for that line before matching the hunk body; it is not a unified-diff line-number header. Hunks match in file order. Context-only hunks are allowed and advance the search position, but a hunk body cannot be empty.', '用相邻的未修改行定位修改。可在 @@ 后加一行原文（例如函数声明），先定位该行，再匹配后面的正文；这里不是 unified diff 的行号头。各区块按文件顺序匹配。允许只有上下文的区块，它会推进查找位置，但区块正文不能为空。'),
        text('Place *** End of File after a hunk body only when its source lines must match the end of the file. To rename, put *** Move to: <new path> immediately after *** Update File. *** Delete File has no body.', '仅当区块中的原文必须匹配文件末尾时，在正文后使用 *** End of File。重命名时，紧接 *** Update File 写 *** Move to: <新路径>。*** Delete File 后没有正文。'),
        text('Example patch value:', 'patch 字段示例：'),
        '*** Begin Patch',
        '*** Add File: notes.txt',
        '+hello',
      ...
      ].join('\n'),
      {
        patch: {
          type: 'string',
          description: text('AppServer patch text with newline-separated lines, from *** Begin Patch through *** End Patch. Follow the format and example in the tool description.', '以换行分隔的 AppServer 补丁文本，从 *** Begin Patch 到 *** End Patch。格式与示例见工具说明。'),
        },
        environment_id: {
          type: 'string',
    ...
    localTool(
      'write_file',
      text('Create or completely overwrite one UTF-8 text file in the local workspace. Before overwriting an existing file, read_file must return its complete, untruncated current contents in this turn (omit range arguments). Wait for the read result before generating the overwrite; a read in the same response cannot authorize it. Any intervening write requires a fresh read. Prefer edit or apply_patch for targeted changes and files too large to read in full.', '创建或完整覆盖一个工作区 UTF-8 文本文件。覆盖现有文件前，本轮必须先用 read_file 读取完整、未截断的当前内容（不指定范围），收到读取结果后再生成覆盖请求；同一回复中的读取不能授权覆盖。期间有写入则必须重新读取。定向修改或无法完整读取的大文件使用 edit 或 apply_patch。'),
      {
        file_path: {
          type: 'string',
          description: text('File path, absolute or relative to the workspace root.', "文件路径，可为绝对路径或相对于工作区根目录的路径。"),
