...
export async function fetchAvailableModels(
  ...
  const timeout = setTimeout(
    () => controller.abort(new Error('模型列表请求超时。')),
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(detail
        ? `模型服务返回异常状态：${response.status} - ${detail}`
        : `模型服务返回异常状态：${response.status}`);
    }
...
function modelListUrl(provider: ModelProviderKind, baseUrl: string): string {
  ...
  if (!base) throw new ModelDiscoveryInputError('请先填写模型服务地址。');
  ...
  try {
    ...
    throw new ModelDiscoveryInputError('服务地址格式不正确。');
  }
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new ModelDiscoveryInputError('模型服务地址只支持 http 或 https。');
  }
  return endpoint;
}

export class ModelDiscoveryInputError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'ModelDiscoveryInputError';
  }
}
...
function parseAvailableModels(value: unknown): RuntimeAvailableModel[] {
  ...
  const items = arrayValue(value) ?? arrayValue(record.data) ?? arrayValue(record.models) ?? [];
...
function parseAvailableModel(value: unknown): RuntimeAvailableModel | null {
  ...
  return { id, name, ...availableModelCapabilities(object) };
}
...
function availableModelCapabilities(object: Record<string, unknown>): Partial<RuntimeAvailableModel> {
  ...
  const thinkingEnabled = booleanValue(
    object.thinkingEnabled ?? object.supportsThinking ?? object.supports_thinking
      ?? object.reasoning ?? object.supportsReasoning ?? object.supports_reasoning,
  ) ?? booleanValue(capabilities.thinking ?? capabilities.reasoning);
  const supportsImages = booleanValue(
    object.supportsImages ?? object.supports_images ?? object.vision
      ?? object.supportsVision ?? object.supports_vision,
  ) ?? booleanValue(capabilities.images ?? capabilities.image ?? capabilities.vision)
    ?? modalityListIncludesImage(
      object.modalities ?? object.inputModalities ?? object.input_modalities
        ?? capabilities.modalities ?? capabilities.inputModalities ?? capabilities.input_modalities,
    );
  const contextWindowTokens = numberValue(
    object.contextWindowTokens ?? object.context_window_tokens ?? object.contextWindow
      ?? object.context_window ?? object.contextLength ?? object.context_length
      ?? object.maxContextTokens ?? object.max_context_tokens ?? object.inputTokenLimit
      ?? object.input_token_limit ?? capabilities.contextWindowTokens
      ?? capabilities.context_window_tokens ?? capabilities.contextWindow
      ?? capabilities.context_window ?? capabilities.maxContextTokens ?? capabilities.max_context_tokens,
  );
  const maxOutputTokens = numberValue(
    object.maxOutputTokens ?? object.max_output_tokens ?? object.max_tokens
      ?? object.outputTokenLimit ?? object.output_token_limit ?? capabilities.maxOutputTokens
      ?? capabilities.max_output_tokens ?? capabilities.max_tokens,
  );
  return {
...
function compactHttpErrorBody(text: string): string {
  try {
    ...
    return trimChars(
      nonEmptyStringValue(error.message) ?? nonEmptyStringValue(value.message)
        ?? nonEmptyStringValue(value.error) ?? text.trim(),
      180,
    );
  } catch {
...
function stringListValue(value: unknown): string[] {
  if (Array.isArray(value)) {
    return value.map(nonEmptyStringValue).filter((item): item is string => Boolean(item));
  }
  if (typeof value === 'string') {
...
function modalityListIncludesImage(value: unknown): boolean | undefined {
  ...
  return values.some((item) => ['image', 'images', 'vision', 'input_image'].includes(item));
}
