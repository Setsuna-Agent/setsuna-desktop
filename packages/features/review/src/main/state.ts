...
export async function getDesktopReviewState(workspaceRoot: string, options: DesktopReviewStateOptions = {}): Promise<DesktopReviewState> {
  ...
  const includeBranchSummary = options.includeBranchSummary !== false;
  ...
  const [branchSummary, fallbackCurrentRemoteSummary, stagedSummary, unstagedSummary] = await Promise.all([
    includeBranchSummary && baseRef ? branchDiffSummary(gitRoot, baseRef) : Promise.resolve(null),
    includeBranchSummary && currentRemoteRef && currentRemoteRef !== baseRef
      ? branchDiffSummary(gitRoot, currentRemoteRef)
