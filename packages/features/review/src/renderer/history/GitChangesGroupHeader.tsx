...
export function GitChangesGroupHeader({ group, collapsed, busy, onToggle, onOpen, onAction }: {
  ...
  return (
    <div className="git-history-section-heading git-changes-files__group-heading">
      <button type="button" className="git-changes-files__group-toggle" aria-expanded={!collapsed} title={[group.description, group.label].filter(Boolean).join(' · ')} onClick={onToggle}>
      ...
      {isWorktree ? (
        <span className="git-changes-group__actions">
          <ReviewIconButton label={t('feature.review.history.openGroup')} onClick={onOpen}><Files size={14} /></ReviewIconButton>
          {group.id === 'unstaged' ? (
            <ReviewIconButton label={t('feature.review.history.discardGroup')} disabled={busy} onClick={() => onAction('discard')}><Undo2 size={14} /></ReviewIconButton>
          ) : null}
          <ReviewIconButton label={t(group.id === 'staged' ? 'feature.review.history.unstageGroup' : 'feature.review.history.stageGroup')} disabled={busy} onClick={() => onAction(group.id === 'staged' ? 'unstage' : 'stage')}>
