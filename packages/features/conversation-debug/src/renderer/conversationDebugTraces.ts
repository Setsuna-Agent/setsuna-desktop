...
export function conversationDebugReplayMessageCount(
  traces: readonly RuntimeDebugTraceEvent[],
): number {
  return new Set(traces.flatMap((trace) => (
    trace.kind === 'provider.replay.decision' ? [trace.payload.messageId] : []
  ))).size;
}
...
function projectDebugTraceNodes(traces: RuntimeDebugTraceEvent[]): ConversationDebugNode[] {
  ...
  for (const trace of traces) {
    ...
    const groupId = providerReplayGroupId(trace);
 ...
 * message and every sampling step can replay that same history again. The flow
 * needs one turn-level node; the inspector still retains every raw trace.
 */
function providerReplayGroupId(trace: ProviderReplayDebugTrace): string {
  return [
...
function providerReplayGroupSummary(traces: ProviderReplayDebugTrace[]): string {
  const messageCount = conversationDebugReplayMessageCount(traces);
  ...
  const requestCount = new Set(traces.map((trace) => trace.spanId).filter(Boolean)).size;
