...
export function conversationDebugMessageToolInstanceId(
  event: Extract<StoredThreadEvent, { type: 'message.completed' }>,
  toolCallId: string,
): string {
  return toolInstanceId(
    `${eventTurnScope(event)}:${event.payload.messageId}`,
    toolCallId,
  );
}
