...
import { mergeConversationDebugTraces } from '../../src/renderer/conversationDebugTraces.js';
...
describe('conversation debug graph', () => {
  ...
  it('groups all provider replay decisions in one turn while retaining raw traces', () => {
    const eventGraph = projectConversationDebugGraph(runtimeEvents());
    const traces = [
    ...
    const replayNode = replayNodes[0];

    expect(replayNodes).toHaveLength(1);
    expect(replayNode).toMatchObject({
      seqStart: 10,
      seqEnd: 13,
      status: 'warning',
      summary: '3 messages · 2 requests · 3 semantic · 1 native',
      traceIds: ['replay_1', 'replay_2', 'replay_3', 'replay_4'],
    });
    expect(new Set(traces.map((trace) => graph.traceNodeIds.get(trace.id)))).toEqual(
      new Set([replayNode?.id]),
    );
    expect(graph.traceNodeIds.get('replay_1')).toBe(replayNode?.id);
    expect(graph.traceNodeIds.get('replay_2')).toBe(replayNode?.id);
    expect(graph.traceNodeIds.get('replay_3')).toBe(replayNode?.id);
    expect(graph.traceNodeIds.get('replay_4')).toBe(replayNode?.id);
