...
import { InMemoryConversationDebugTraceStore } from '../../src/runtime/in-memory-conversation-debug-trace-store.js';

describe('InMemoryConversationDebugTraceStore', () => {
  it('keeps a bounded sequence-addressable buffer per thread', () => {
    let id = 0;
    const store = new InMemoryConversationDebugTraceStore({
...
function appendTrace(store: InMemoryConversationDebugTraceStore, threadId: string): void {
  store.append({
    afterEventSeq: 0,
