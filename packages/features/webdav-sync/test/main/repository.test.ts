...
import { generateWebDavRecoveryKey, sha256Buffer } from '../../src/main/crypto.js';
...
describe('EncryptedWebDavRepository', () => {
  ...
  it('accepts portable Feature settings generated for the preferences category', async () => {
    const server = new MemoryWebDavServer('/dav');
    const client = new WebDavClient(
      normalizeWebDavLocation({ endpoint: 'https://dav.test/dav', remoteRoot: '/Backups' }),
      { username: 'alice', password: 'secret' },
    ...
    expect(() => repository.validateSnapshotManifest({
      formatVersion: 1,
      repositoryId: repository.metadata.repositoryId,
      id: createSnapshotId(new Date('2026-08-10T12:40:00.000Z')),
      deviceId: '55bc8840-ac7a-435a-b5a7-88c2e91e7d87',
      deviceName: 'Work laptop',
      ...
      items: [{
        ...
        kind: 'file',
        logicalPath: 'runtime/portable-feature-settings/memory/preferences.json',
        label: 'Feature 设置：memory/preferences',
        objectName: '000001.enc',
        sha256: '0'.repeat(64),
        size: 0,
      }],
