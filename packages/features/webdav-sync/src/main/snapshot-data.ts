...
export async function summarizeLocalSnapshotCategories(
  ...
  try {
    for (const source of sources) {
      ...
      if (!summary) continue;
      summary.totalBytes += await localSourceSize(source, input.storage.dataLayout(input.dataRoot));
...
async function collectLocalSnapshotSources(
  ...
  const layout = input.storage.dataLayout(input.dataRoot);
...
async function localSourceSize(
  source: LocalSnapshotSource,
  layout: ReturnType<WebDavSyncStorageHost['dataLayout']>,
): Promise<number> {
...
export function categoryTargetPaths(
  ...
  const layout = storage.dataLayout(dataRoot);
