...
import { isPortableFeatureSettingsLogicalPath } from './portable-feature-settings.js';
import {
  type WebDavClient,
...
function allowedFilePath(category: DesktopWebDavSyncCategoryId, logicalPath: string): boolean {
  ...
  if (category === 'preferences') {
    return logicalPath === 'runtime/config.json'
      || isPortableFeatureSettingsLogicalPath(logicalPath);
  }
