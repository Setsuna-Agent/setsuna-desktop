...
export async function applyRestoredSnapshot(input: {
  ...
  await input.storage.relocateDataRootContents(
    input.stagingRoot,
  ...
  const targets = [...new Set([
    ...categoryTargetPaths(input.dataRoot, input.categories, input.storage),
  ...
  try {
    ...
    await writeWebDavRestoreJournal(input.dataRoot, journal, input.storage);
    journalWritten = true;
    ...
    await writeWebDavRestoreJournal(
      input.dataRoot,
    ...
    try {
      await syncWebDavRestorePathParents(input.dataRoot, rollbackChangedPaths, input.storage);
    } catch (currentError) {
      rollbackError ??= currentError;
    }
    rollbackComplete = rollbackError === undefined;
    if (rollbackComplete && journalWritten) {
      await clearWebDavRestoreJournal(input.dataRoot, input.storage).catch(() => undefined);
    }
    if (rollbackError) {
