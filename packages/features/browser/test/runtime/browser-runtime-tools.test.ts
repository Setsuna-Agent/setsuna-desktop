...
describe('BrowserRuntimeTools', () => {
  it('exposes an unapproved browser tool with a structured UI action', async () => {
    const host = new BrowserRuntimeTools();
  ...
  it('exposes page controls when the Electron bridge is available', async () => {
    ...
    const host = new BrowserRuntimeTools(control);
    ...
    expect(systemPrompt).toContain('Successful browser input calls confirm dispatch only');
  ...
  it('does not expose or run browser screenshots for text-only models', async () => {
    const host = new BrowserRuntimeTools({ execute: async () => ({ kind: 'tabs', tabs: [] }) });
