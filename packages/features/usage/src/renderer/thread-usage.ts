...
function aggregateStoredUsage(records: readonly RuntimeUsageRecord[]): Map<string, UsageAggregate> {
...
function addRecordsToSummary(
  ...
  const next: MutableUsageSummary = {
...
type MutableUsageSummary = {
  inputTokens: number;
  cachedInputTokens: number;
  outputTokens: number;
  totalTokens: number;
  recordCount: number;
  byDay: MutableUsageBucket[];
  byProvider: MutableUsageBucket[];
  byModel: MutableUsageBucket[];
};

type MutableUsageBucket = {
  -readonly [TKey in keyof RuntimeUsageBucket]: RuntimeUsageBucket[TKey];
};

function addToBucket(buckets: MutableUsageBucket[], key: string, usage: RuntimeUsage, recordCount: number): void {
