...
/**
 * Merges static metadata from every installed module, including disabled modules.
...
export function composeRendererMessages<const TLocale extends string>(
  ...
  for (const [locale, entries] of Object.entries(hostMessages) as [TLocale, Readonly<Record<string, string>>][]) {
    ...
    for (const key of localeCatalog.keys()) keyOwners.set(key, 'host');
  }
  ...
  for (const { module } of mounts) {
    for (const bundle of module.messages) {
      ...
      if (existingOwner) {
        throw new Error(
          `Renderer message namespace "${bundle.namespace}" is owned by both "${existingOwner}" and "${module.definition.id}".`,
        );
      }
      ...
      for (const [locale, entries] of Object.entries(bundle.messages)) {
        ...
        for (const [key, value] of Object.entries(entries)) {
          const existingKeyOwner = keyOwners.get(key);
          if (existingKeyOwner) {
            throw new Error(
              `Renderer message key "${key}" from "${module.definition.id}" conflicts with "${existingKeyOwner}".`,
            );
          }
      ...
      for (const entries of Object.values(bundle.messages)) {
        if (!entries) continue;
        for (const key of Object.keys(entries)) keyOwners.set(key, module.definition.id);
      }
    }
  }
