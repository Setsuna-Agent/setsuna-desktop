...
describe('createComposerDraftSyncPlan', () => {
  ...
  it('ignores an effect replay while an editor change is waiting for its parent echo', () => {
    expect(createComposerDraftSyncPlan('', '', '对话创建插件 ', '对话创建插件 ')).toEqual({ type: 'none' });
  });
  ...
  it('adopts a draft that is already visible in the editor', () => {
    expect(createComposerDraftSyncPlan('visible draft', 'previous draft', 'stale draft', 'visible draft')).toEqual({ type: 'adopt' });
  });

  it('appends external additions without rebuilding existing slots', () => {
    expect(createComposerDraftSyncPlan('/review existing draft\n@src/main.ts ', '/review existing draft', '/review existing draft', '/review existing draft')).toEqual({
      type: 'append',
      value: '\n@src/main.ts ',
    });
  });
  ...
  it('replaces the editor for an unrelated external draft', () => {
    expect(createComposerDraftSyncPlan('restored draft', 'old draft', 'old draft', 'old draft')).toEqual({
      type: 'replace',
  ...
  it('clears the editor when the external draft is reset', () => {
    expect(createComposerDraftSyncPlan('', 'old draft', 'old draft', 'old draft')).toEqual({ type: 'replace', value: '' });
  });
});
