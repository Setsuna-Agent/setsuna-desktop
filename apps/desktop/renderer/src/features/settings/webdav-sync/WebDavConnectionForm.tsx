import type {
  DesktopWebDavSyncConfigureInput,
  DesktopWebDavSyncRepositoryMode,
} from '@setsuna-desktop/contracts';
import { Check, KeyRound, Server } from 'lucide-react';
import { useState, type FormEvent } from 'react';
import { useI18n } from '../../../shared/i18n/I18nProvider.js';
import { Button, SelectField, TextField } from '../../../shared/ui/primitives.js';
import { WebDavSecretField } from './WebDavSecretField.js';

type ConnectionDraft = {
  endpoint: string;
  remoteRoot: string;
  username: string;
  password: string;
  recoveryKey: string;
  deviceName: string;
  repositoryMode: DesktopWebDavSyncRepositoryMode;
  allowInsecureHttp: boolean;
};

export function WebDavConnectionForm({
  disabled,
  onSubmit,
  onTest,
}: {
  disabled: boolean;
  onSubmit: (input: DesktopWebDavSyncConfigureInput) => Promise<void>;
  onTest: (input: DesktopWebDavSyncConfigureInput) => Promise<void>;
}) {
  const { t } = useI18n();
  const [draft, setDraft] = useState<ConnectionDraft>(() => ({
    endpoint: '',
    remoteRoot: '/setsuna',
    username: '',
    password: '',
    recoveryKey: '',
    deviceName: '',
    repositoryMode: 'create',
    allowInsecureHttp: false,
  }));
  const [submitting, setSubmitting] = useState(false);
  const [testing, setTesting] = useState(false);
  const [testSucceeded, setTestSucceeded] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const busy = disabled || submitting || testing;

  function updateDraft<Key extends keyof ConnectionDraft>(
    key: Key,
    value: ConnectionDraft[Key],
  ) {
    setDraft((current) => ({ ...current, [key]: value }));
    setError(null);
    setTestSucceeded(false);
  }

  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (busy) return;
    setSubmitting(true);
    setError(null);
    setTestSucceeded(false);
    try {
      await onSubmit(configureInput(draft));
    } catch (unknownError) {
      setError(unknownError instanceof Error ? unknownError.message : String(unknownError));
    } finally {
      setSubmitting(false);
    }
  };

  const testConnection = async () => {
    if (busy || !canTestConnection(draft)) return;
    setTesting(true);
    setError(null);
    setTestSucceeded(false);
    try {
      await onTest(configureInput(draft));
      setTestSucceeded(true);
    } catch (unknownError) {
      setError(unknownError instanceof Error ? unknownError.message : String(unknownError));
    } finally {
      setTesting(false);
    }
  };

  return (
    <form className="settings-webdav__connection-form settings-webdav__card" onSubmit={(event) => void submit(event)}>
      <header className="settings-webdav__card-header">
        <span className="settings-webdav__card-icon"><Server size={16} /></span>
        <div>
          <strong>{t('settings.sync.connection.title')}</strong>
          <small>{t('settings.sync.connection.description')}</small>
        </div>
      </header>
      <div className="settings-webdav__form-grid">
        <label className="settings-webdav__field">
          <span>{t('settings.sync.connection.mode')}</span>
          <SelectField
            disabled={busy}
            value={draft.repositoryMode}
            onValueChange={(value) => updateDraft(
              'repositoryMode',
              value === 'connect' ? 'connect' : 'create',
            )}
          >
            <option value="create">{t('settings.sync.connection.create')}</option>
            <option value="connect">{t('settings.sync.connection.connect')}</option>
          </SelectField>
        </label>
        <label className="settings-webdav__field settings-webdav__field--wide">
          <span>{t('settings.sync.connection.endpoint')}</span>
          <TextField
            autoFocus
            disabled={busy}
            required
            spellCheck={false}
            type="url"
            value={draft.endpoint}
            placeholder={t('settings.sync.connection.endpointPlaceholder')}
            onChange={(event) => updateDraft('endpoint', event.target.value)}
          />
        </label>
        <label className="settings-webdav__field">
          <span>{t('settings.sync.connection.remoteRoot')}</span>
          <TextField
            disabled={busy}
            required
            spellCheck={false}
            value={draft.remoteRoot}
            placeholder={t('settings.sync.connection.remoteRootPlaceholder')}
            onChange={(event) => updateDraft('remoteRoot', event.target.value)}
          />
        </label>
        <label className="settings-webdav__field">
          <span>{t('settings.sync.connection.deviceName')}</span>
          <TextField
            disabled={busy}
            value={draft.deviceName}
            placeholder={t('settings.sync.connection.deviceNamePlaceholder')}
            onChange={(event) => updateDraft('deviceName', event.target.value)}
          />
        </label>
        <label className="settings-webdav__field">
          <span>{t('settings.sync.connection.username')}</span>
          <TextField
            autoComplete="username"
            disabled={busy}
            required
            value={draft.username}
            onChange={(event) => updateDraft('username', event.target.value)}
          />
        </label>
        <div className="settings-webdav__field">
          <label htmlFor="settings-webdav-password">{t('settings.sync.connection.password')}</label>
          <WebDavSecretField
            id="settings-webdav-password"
            autoComplete="current-password"
            disabled={busy}
            required
            value={draft.password}
            onChange={(event) => updateDraft('password', event.target.value)}
          />
        </div>
        {draft.repositoryMode === 'connect' ? (
          <div className="settings-webdav__field settings-webdav__field--wide">
            <label htmlFor="settings-webdav-recovery-key">{t('settings.sync.connection.recoveryKey')}</label>
            <WebDavSecretField
              id="settings-webdav-recovery-key"
              autoComplete="off"
              disabled={busy}
              leadingIcon={<KeyRound size={14} />}
              required
              spellCheck={false}
              value={draft.recoveryKey}
              placeholder={t('settings.sync.connection.recoveryKeyPlaceholder')}
              onChange={(event) => updateDraft('recoveryKey', event.target.value)}
            />
          </div>
        ) : null}
      </div>
      <label className="settings-webdav__insecure-toggle">
        <input
          checked={draft.allowInsecureHttp}
          disabled={busy}
          type="checkbox"
          onChange={(event) => updateDraft('allowInsecureHttp', event.currentTarget.checked)}
        />
        <span>
          <strong>{t('settings.sync.connection.insecure')}</strong>
          <small>{t('settings.sync.connection.httpWarning')}</small>
        </span>
      </label>
      {error ? <div className="settings-webdav__error" role="alert">{error}</div> : null}
      {testSucceeded ? (
        <div className="settings-webdav__success" role="status">
          <Check size={13} />{t('settings.sync.connection.testSuccess')}
        </div>
      ) : null}
      <footer className="settings-webdav__form-actions">
        <small>{t('settings.sync.connection.testHint')}</small>
        <span>
          <Button
            disabled={busy || !canTestConnection(draft)}
            onClick={() => void testConnection()}
          >
            {testing ? t('settings.sync.connection.testing') : t('settings.sync.connection.test')}
          </Button>
          <Button disabled={busy} type="submit" variant="primary">
            {submitting
              ? t('common.processing')
              : t(draft.repositoryMode === 'create'
                ? 'settings.sync.connection.createAction'
                : 'settings.sync.connection.connectAction')}
          </Button>
        </span>
      </footer>
    </form>
  );
}

function configureInput(draft: ConnectionDraft): DesktopWebDavSyncConfigureInput {
  return {
    endpoint: draft.endpoint,
    remoteRoot: draft.remoteRoot,
    username: draft.username,
    password: draft.password,
    repositoryMode: draft.repositoryMode,
    allowInsecureHttp: draft.allowInsecureHttp,
    ...(draft.repositoryMode === 'connect' ? { recoveryKey: draft.recoveryKey } : {}),
    ...(draft.deviceName.trim() ? { deviceName: draft.deviceName } : {}),
  };
}

function canTestConnection(draft: ConnectionDraft): boolean {
  return Boolean(
    draft.endpoint.trim()
    && draft.remoteRoot.trim()
    && draft.username.trim()
    && draft.password
    && (draft.repositoryMode === 'create' || draft.recoveryKey.trim()),
  );
}
