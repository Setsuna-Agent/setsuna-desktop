...
export function UsageTimeRangeFilter({
  ...
  return (
    <section className="settings-usage-time-filter" aria-label={t('settings.usage.timeRange')}>
      <div className="settings-usage-time-filter__main">
        <div className="settings-usage-time-filter__presets" role="group" aria-label={t('settings.usage.quickRanges')}>
          {PRESETS.map((preset) => (
            <button
...
function presetLabel(preset: UsageTimePreset, t: Translate): string {
  if (preset === 'all') return t('settings.usage.allTime');
  if (preset === 'today') return t('settings.usage.today');
