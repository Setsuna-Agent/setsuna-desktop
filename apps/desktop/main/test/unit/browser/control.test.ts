...
import type {
  ...
  BrowserTypeOptions,
} from '../../../src/browser/cdp/automation.js';
import { DesktopBrowserController } from '../../../src/browser/control.js';
...
describe('DesktopBrowserController', () => {
  ...
  it('invalidates old snapshot refs and routes real-input commands through the tab adapter', async () => {
    ...
    await expect(controller.execute({ key: 'Tab', kind: 'key', repeat: 2 })).resolves.toMatchObject({
      message: 'Dispatched Tab 2 times using Electron native keyboard input.',
    });
    expect(contents.focusCount).toBe(4);
    expect(contents.hostFocusCount).toBe(4);
    expect(contents.inputEvents).toEqual([
      { keyCode: 'Tab', modifiers: [], type: 'rawKeyDown' },
      { keyCode: 'Tab', modifiers: [], type: 'keyUp' },
      { keyCode: 'Tab', modifiers: [], type: 'rawKeyDown' },
      { keyCode: 'Tab', modifiers: [], type: 'keyUp' },
    ]);
  ...
  it('dispatches direction keys through the Electron guest input channel', async () => {
    const automation = new FakeAutomation();
    ...
    await expect(controller.execute({
      ...
      message: 'Dispatched Control+Shift+ArrowRight 2 times using Electron native keyboard input.',
      tabId: 'game',
    });

    expect(contents.inputEvents).toEqual([
      { keyCode: 'Right', modifiers: ['control', 'shift'], type: 'rawKeyDown' },
      { keyCode: 'Right', modifiers: ['control', 'shift'], type: 'keyUp' },
      { keyCode: 'Right', modifiers: ['control', 'shift'], type: 'rawKeyDown' },
      { keyCode: 'Right', modifiers: ['control', 'shift'], type: 'keyUp' },
    ]);
    expect(contents.hostFocusCount).toBe(1);
