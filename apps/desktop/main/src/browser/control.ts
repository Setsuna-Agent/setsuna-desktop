...
export class DesktopBrowserController implements BrowserControlExecutor {
  ...
  private async key(
    ...
    const [tabId, entry] = this.resolveTab(requestedTabId);
    this.focusForInput(entry, signal);
    const message = dispatchNativeBrowserKey(entry.contents, {
  ...
  private focusForInput(entry: RegisteredBrowserTab, signal?: AbortSignal): void {
    throwIfAborted(signal);
    // 先恢复宿主页面焦点，再把焦点交给 webview guest，避免原生事件停在聊天输入框。
    entry.contents.hostWebContents?.focus();
