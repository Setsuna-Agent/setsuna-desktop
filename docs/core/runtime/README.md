...
| `packages/features/*/runtime` | Feature-owned use case、typed operation、settings/event owner 与协议实现 | [Feature inventory](../../features/README.md) |
| `src/ports/` | Runtime 内部抽象 | [Ports 与 adapters](ports-and-adapters.md) |
| `src/adapters/store/` | SQLite、JSON、附件、memory | [存储](storage.md) |
| `src/adapters/model/` | 模型可见图片资产解析 wrapper | [Model Provider Feature](../../features/model-provider.md) |
...
这些基础设施并不代表业务所有权回到 Runtime Core。Factory 创建 store、bindable port 和宿主 adapter，`runtime-feature-composition.ts` 再把它们作为窄 Capability 注入对应 Feature；Feature 激活后把 service/control 绑定回 AgentLoop、ToolHost 或 server 接缝。
...
- Provider： [model-providers.md](../../features/model-provider.md)
- MCP： [mcp.md](../../features/mcp.md)
- 所有 Runtime Feature：[Feature inventory](../../features/README.md)
