...
`desktopSettings.interfaceLanguage` 同时选择界面语言和内置模型文案。采样上下文将它传入 `ToolExecutionContext`，各工具 owner 使用 `runtimeText` 就地维护中英文说明，共用参数结构和执行逻辑。基础指令、环境/权限、Skill 引导、工具策略、工具及参数描述按语言生成；MCP/扩展提供方内容、用户的项目规则、Skill 正文和工具结果保留原文。

已选择的语言是默认回复语言；用户在当前请求或 steer 中明确要求另一语言时覆盖。没有保存语言设置的旧配置仍按用户请求和对话历史推断。每次采样末尾保留高权限的回复语言约束，覆盖工具前后说明、进度、工具参数中的用户可见文案和最终答复。设置变化在下一次采样生效，旧对话无需重建；专用 review 优先根据用户请求确定回复语言，调用方提供的语言只作兜底。
...
压缩交接由 `context-compaction-summary.ts` 构造和校验：保留已完成进度、结论与证据、
剩余缺口及下一步；旧摘要不再按普通工具结果截短。当前 turn 的用户请求和修正在有界预算内
保留原文，工具调用/结果仍按完整事务切分。最终交接文本最多约 4096 tokens，同时受保留
上下文后的剩余空间约束。`context-compaction-request.ts` 单独按实际摘要模型的上下文窗口、
完整摘要输入和模型输出上限计算生成预算；生成预算包含思考，不与最终摘要长度共用上限。
手动和自动压缩复用同一流程，按当前任务模型预留续跑输出及摘要包装空间；最新文本输入与
固定消息合计超出保留预算时，可将该输入纳入摘要。可用摘要空间不足 256 tokens，或完整
输入无法装入摘要模型窗口时，在模型调用前失败并保留原历史。
摘要使用普通交接文本，无需 JSON；仅完整结束、非空且不超长的摘要可替换历史。
失败最多重试一次，仍失败则保留原历史并结束失败路径，不以原文片段冒充成功摘要。
每次摘要或原生压缩调用结束时独立发布 `token.count` 并写入 UsageRecorder；截断、取消或
最终压缩提交失败都不丢弃供应商已返回的用量，同一调用的重复 usage 通知只结算一次。

显式配置摘要任务模型时只调用该模型。否则优先尝试 Responses 原生压缩；原生结果不可用
或过大时退回文本摘要，不在成功的原生压缩之前额外生成摘要。原生检查点通过
`nativeSourceMessageIds` 引用已有 transcript 中的原文；Contracts 的 `event-projections/native-compaction.ts`
在模型、供应商、端点或语义不兼容时先恢复原文，再检查预算和组织请求。恢复可递归展开多次
压缩，缺失原文时明确失败。无法保留原文的临时 model-only 消息使用文本摘要路径。
侧聊复制前递归展开原生检查点，把原文作为独立的 model-only 快照交接，再按侧聊窗口压缩。
删除和截断事件在共享投影中移除受影响的原生检查点，恢复剩余有效原文；不能仅剪掉引用 ID
而继续回放含有已删除内容的 opaque 状态。无关检查点与其归档原文保持不变。

`ContextTokenCalibration` 用最近一次请求的供应商 input usage 补足本地估算的低估部分，
新增消息和新加载的工具继续独立估算；cached input 和账单累计 total 不额外计入。
模型/供应商变化或新的压缩摘要会使旧基准失效。压缩触发和采样快照使用同一预算口径，
输出预留只计算一次；这仍是保守估算，不承诺与供应商 tokenizer 完全一致。
...
普通 send-now 可在可接收输入的 active normal/goal turn 上转 steer；Goal 不能被改写为 steer。详见 [设计文档](../../designs/current/queued-turn-inputs.md)。
...
[持久化 Goal 设计](../../designs/current/persistent-goals.md)。
