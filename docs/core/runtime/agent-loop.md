...
保留原文，工具调用/结果仍按完整事务切分。摘要默认最多 4096 tokens，重试可到 8192，
同时受模型输出上限和保留上下文后的剩余预算约束。手动和自动压缩的候选选择均预留摘要及
包装空间；最新文本输入与固定消息合计超出保留预算时，可将该输入纳入摘要。可用输出不足
256 tokens 时在模型调用前失败，保留原历史，不发送或重试不可用的摘要请求。
仅完整结束、结构有效的摘要可替换历史；
