...
能力页的“插件”分区默认展示随应用发布的精选市场，并分成“发现 / 已安装”两部分。用户按插件 ID 一键安装，renderer 不接触 Bundle 路径或 runtime 安装目录；已安装页负责查看能力摘要和安全卸载。详细 manifest、市场和所有权规则见 `docs/plugin-bundles.md`。
