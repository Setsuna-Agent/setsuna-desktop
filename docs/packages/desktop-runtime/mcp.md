...
renderer composition
  └── feature-mcp/renderer
         ├── typed operation client
         └── MCP snapshot + mutation service
```
...
- MCP management typed operations：server 列表、发现工具、保存、启停、删除与 OAuth 登录/登出。
- `McpRendererService`：renderer snapshot、mutation 和迟到请求收敛。
...
- renderer 管理面通过 `/v1/features/mcp/*` typed operations 调用同一个 `McpControl`，不进入统一 `DesktopRuntimeClient`。
