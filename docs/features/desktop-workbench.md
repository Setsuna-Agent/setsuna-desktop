...
- Feature 生成完整的审查 prompt、developer policy 和模型选择后，才通过窄 `ReviewRuntimeHost.startTurn` 交给 Core。Core 保留通用 turn/event 真源和只检查、不修改代码的工具范围，不解释 Review target。用户明确选择完全访问时，审查 shell 沿用该权限并在 OS 沙箱外执行；其他权限模式仍强制只读沙箱。完全访问下“不修改代码”由审查提示词和工具范围约束，shell 不具有 OS 层的写入隔离。
- 审查优先跟随原始用户请求的回复语言，界面语言仅作兜底；中文审查策略使用中文，并约束进度说明、工具前后说明及最终结果。中途的“立即发送”补充在同一 review turn 的下一采样检查点生效，可调整回复语言，但不改变审查的只检查、不修改代码约束。
- 专用 Review 模型控制该轮采样及压缩预算，包含供应商实际输入 token 对本地估算的修正。超出预算时先生成交接摘要并持久化压缩事件，保留本轮原始请求和近期工具结果，再继续审查；不直接裁掉历史。线程仍绑定当前对话模型，原始消息保留在转录中。
