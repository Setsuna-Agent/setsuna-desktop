...
3. readable roots 向本次 logon SID 临时授予 read/execute；
4. writable roots 同时向 logon SID 临时授予写入，并向 capability 设置稳定写入 ACE；
5. 已存在的受保护元数据根向 capability 添加 write deny；
6. Job Object 结束后回收全部 logon SID 和 request-bootstrap ACE；即使中途配置失败，RAII guard 也执行回收；
7. 每次执行的 `work/` 不进入 capability policy key，避免状态和持久 ACE 随临时目录无限增长；
8. 所有输入路径先 canonicalize，并限制为固定本机 NTFS 卷。

最终 shell 使用 `CreateRestrictedToken(DISABLE_MAX_PRIVILEGE | LUA_TOKEN | WRITE_RESTRICTED)`。默认 DACL
和 restricting SID 列表都只包含 capability SID 与本次 logon SID；读取仍按普通 token 检查，写入必须再由
这两个窄 SID 之一命中，不能借用 Everyone 对宿主路径的宽泛写权限。外层 account runner 从机器级只读副本
启动，并被放入不可 breakaway、close 即 kill 的 Job Object。
...
`NO_PROXY`；随机 loopback relay 凭据只短暂存在于 control request，且在最终 shell 启动前删除。
...
防火墙被组策略覆盖、活动 profile 被关闭、代理端口无法完整占用、配置了上游代理、路径无法由 NTFS ACL
