...
| 层 | 职责 |
| --- | --- |
| `packages/features/webdav-sync/src/contracts` | WebDAV config/state、category、snapshot summary、restore plan/diff、固定 IPC channel 和 preload bridge contract。 |
| `packages/features/webdav-sync/src/main` | 配置、客户端、加密仓库、数据域/路径边界、调度、restore plan、回滚日志、IPC 与 relaunch 编排；不持有业务 Feature settings schema 清单。 |
| `packages/features/webdav-sync/src/preload` | 固定 IPC 方法和 state-change subscription；不暴露 raw `ipcRenderer`。 |
| `packages/features/webdav-sync/src/renderer` | “同步”设置 contribution、连接表单、数据域选择、进度、当前备份、还原损失清单、文案与作用域样式。 |
