...
1. Main 先停止 Updater 定时检查、WebDAV 调度和 browser proxy 等新的外围工作。
...
5. Main dispose Feature composition；scope 先拒绝新 IPC，排空已进入的操作，再关闭 Browser server、Terminal PTY 和 Updater 传输并撤销 handler。
6. 关闭 native bridge 与网络代理资源。
