...
4. `Codex Review Gate` 接受 `chatgpt-codex-connector[bot]` 针对当前 HEAD 的标准 review、带 SHA 的 clean-review 顶层评论，或安全 review generation 中产生的 PR 顶层 👍 reaction。初次 review 建立 generation；只有前一 HEAD 已由真实 Codex 结果终结，新 HEAD 才继承 reaction 识别能力，避免旧 review 的延迟 reaction 放行新提交。
