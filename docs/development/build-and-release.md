...
4. `Codex Review Gate` 接受 `chatgpt-codex-connector[bot]` 针对当前 HEAD 的标准 review、带 SHA 的 clean-review 顶层评论，或安全 review generation 中产生的 PR 顶层 👍 reaction。workflow 从 Actions run 历史与 commit status 恢复 generation：draft-only 的 skipped run 不启动 review，可以忽略；其他无 status 的 run 会中断安全 lineage。只有前一 generation 已由真实 Codex 结果终结，新 HEAD 才继承 reaction 识别能力，避免 draft/连续 push 下旧 review 的延迟 reaction 放行新提交。
