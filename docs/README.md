...
2. [运行链路](architecture/runtime-flows.md)：启动、REST、SSE、Agent turn 和浏览器控制。
3. [Desktop 应用](apps/desktop/README.md)：Electron main、preload、React renderer 如何协作。
4. [共享契约](packages/contracts/README.md)：跨进程 DTO、线程事件和 client contract。
5. [本地 Runtime](packages/desktop-runtime/README.md)：server、Agent loop、ports/adapters 和持久化。
6. 根目录 [Tree.md](../Tree.md)：需要继续定位具体文件时再查生成的目录索引。
...
| 新增 preload / IPC 能力 | [Runtime 与 IPC](apps/desktop/main/runtime-and-ipc.md)、[preload](apps/desktop/preload/README.md) | main `src/ipc/`、preload `src/index.ts`、contracts `desktop.ts` |
...
| Runtime REST API | [Server](packages/desktop-runtime/server.md)、[传输契约](packages/contracts/transport-and-data.md) | contracts `http.ts`、runtime `runtime-rest-routes.ts`、renderer `client.ts` |
| 线程事件或消息投影 | [线程与事件](packages/contracts/threads-and-events.md) | contracts `events.ts`、`thread-events.ts`、runtime event writer/store、renderer `runtimeEvents.ts` |
...
| 模型供应商或协议回放 | [模型适配器](packages/desktop-runtime/model-providers.md) | runtime `src/adapters/model/` |
